...
function Score({ text }: { text: string }) {
  ...
  return (
    <div className="h-24 w-24">
      <CircularProgressbar value={score} text={`${score}`} styles={buildStyles({ textColor: '#fff', pathColor: '#fbbf24', trailColor: 'rgba(255,255,255,0.08)' })} />
    </div>
  );
}
...
function TemplatePicker({ value, onChange }: { value: SpeechTemplateId | null; onChange: (id: SpeechTemplateId | null) => void }) {
  ...
  return (
    <div className="grid gap-4">
      <Shell>
        <p className="mb-3 font-mono text-[11px] uppercase tracking-[0.3em] text-slate-400">Speech Format</p>
        <Label.Root className="mb-2 block text-sm text-slate-300">Evaluation rubric</Label.Root>
        <Select.Root value={value ?? 'general'} onValueChange={(next) => onChange(next === 'general' ? null : (next as SpeechTemplateId))}>
          <Select.Trigger className="flex h-14 w-full items-center justify-between rounded-[18px] border border-white/12 bg-slate-950/40 px-4 text-left text-sm text-white sm:rounded-[22px] sm:px-5">
            <Select.Value placeholder="General evaluation" />
            <Select.Icon><ChevronDown className="h-4 w-4 text-slate-400" /></Select.Icon>
          </Select.Trigger>
          <Select.Portal>
              <Select.Content position="popper" className="z-50 max-w-[calc(100vw-2rem)] overflow-hidden rounded-3xl border border-white/10 bg-slate-950/95 p-2 text-white">
              <Select.Viewport className="grid gap-1">
                <Select.Item value="general" className="flex cursor-pointer items-center justify-between rounded-2xl px-4 py-3 text-sm outline-none hover:bg-white/10">
                  ...
                  <Select.ItemIndicator><Check className="h-4 w-4 text-amber-300" /></Select.ItemIndicator>
                </Select.Item>
    ...
    <div className="min-h-screen overflow-x-hidden bg-[radial-gradient(circle_at_top_left,rgba(251,191,36,0.15),transparent_22%),radial-gradient(circle_at_top_right,rgba(124,58,237,0.24),transparent_30%),linear-gradient(180deg,#060816_0%,#0b1020_45%,#131a2d_100%)] text-white">
      ...
      <div className="mx-auto flex min-h-screen max-w-[1440px]">
        ...
        <aside className={cn('fixed inset-y-0 left-0 z-40 w-[84vw] max-w-72 border-r border-white/10 bg-slate-950/90 p-4 backdrop-blur-xl transition md:static md:w-72 md:max-w-none md:translate-x-0 md:p-5 lg:w-80', sidebarOpen ? 'translate-x-0' : '-translate-x-full')}>
          <div className="rounded-[28px] border border-white/10 bg-[linear-gradient(135deg,rgba(251,191,36,0.10),rgba(124,58,237,0.16))] p-5">
            <div className="font-serif text-4xl tracking-[-0.04em]">Aawaz</div>
            <div className="font-mono text-[10px] uppercase tracking-[0.3em] text-slate-300">Speaker Coach</div>
          </div>
          <div className="mt-6 grid gap-2">
            {navItems.map(({ id, label, icon: Icon }) => (
              <button key={id} onClick={() => { setActiveTab(id as Tab); setSidebarOpen(false); }} className={cn('flex items-center gap-3 rounded-[20px] px-4 py-4 text-left transition sm:rounded-[24px]', activeTab === id ? 'bg-white/10' : 'hover:bg-white/6')}>
                <span className="flex h-10 w-10 items-center justify-center rounded-2xl bg-white/10"><Icon className="h-4 w-4" /></span>
          ...
          <AnimatePresence mode="wait">
            <motion.div key={activeTab} initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }} className="grid gap-5">
                  ...
                  <Shell>
                    <div className="grid gap-6 lg:grid-cols-[1fr,auto] lg:items-center">
                      <div className="space-y-4">
                        <div className="rounded-[20px] border border-white/10 bg-slate-950/35 p-4 sm:rounded-[24px] sm:p-5">
                        ...
                        {isRecording && (
                          <div className="flex h-14 items-center gap-1 overflow-hidden rounded-[20px] border border-white/10 bg-white/6 px-3 sm:h-16 sm:rounded-[24px] sm:px-4">
                            {Array.from({ length: 30 }).map((_, index) => (
                              <motion.div key={index} animate={{ scaleY: [0.35, 1, 0.4] }} transition={{ duration: 0.7, repeat: Infinity, delay: index * 0.03 }} className={cn('w-1 rounded-full', index % 3 === 0 ? 'bg-amber-300' : 'bg-violet-300')} style={{ height: 42 }} />
                            ))}
                          </div>
                        )}
                      </div>
                      <div className="flex flex-col items-center gap-4">
                        <motion.button whileTap={{ scale: 0.96 }} animate={isRecording ? { scale: [1, 1.05, 1] } : { scale: 1 }} transition={{ duration: 1.8, repeat: isRecording ? Infinity : 0 }} onClick={isRecording ? stopRecording : startRecording} disabled={isAnalyzing} className={cn('flex h-28 w-28 items-center justify-center rounded-full border text-white shadow-[0_24px_60px_rgba(2,6,23,0.45)] sm:h-32 sm:w-32 lg:h-36 lg:w-36', isRecording ? 'bg-[linear-gradient(135deg,#7f1d1d,#ef4444)]' : 'bg-[linear-gradient(135deg,#0f172a,#6d28d9)]')}>
                          {isAnalyzing ? <Sparkles className="h-10 w-10 animate-spin" /> : isRecording ? <MicOff className="h-10 w-10" /> : <Mic className="h-10 w-10" />}
                        </motion.button>
                        <div className="text-center font-mono text-[10px] uppercase tracking-[0.24em] text-slate-400 sm:text-[11px] sm:tracking-[0.3em]">{isAnalyzing ? 'Analyzing' : isRecording ? 'Tap to stop' : 'Tap to speak'}</div>
                      </div>
                    </div>
                  </Shell>
              ...
              {activeTab === 'speech' && (
                <>
                  <Shell>
                    <Label.Root htmlFor="speech-topic" className="mb-2 block text-sm text-slate-300">Speech topic</Label.Root>
                    <div className="grid gap-4 md:grid-cols-[1fr,auto]">
                      <input id="speech-topic" value={topic} onChange={(event) => setTopic(event.target.value)} onKeyDown={(event) => event.key === 'Enter' && generateSpeech()} placeholder="e.g. Leadership, climate change, discipline" className="h-14 min-w-0 rounded-[18px] border border-white/12 bg-slate-950/40 px-4 text-sm text-white outline-none sm:rounded-[22px] sm:px-5" />
                  ...
                  <Shell>
                    <div className="grid gap-3">
                      {history.length ? history.map((item, index) => (
                        <motion.button key={item.id} whileHover={{ y: -2 }} onClick={() => setSelectedSessionId(selectedSessionId === item.id ? null : item.id)} className={cn('w-full rounded-[20px] border p-4 text-left sm:rounded-[24px] sm:p-5', selectedSessionId === item.id ? 'border-amber-300/40 bg-white/10' : 'border-white/10 bg-white/4')}>
                          <div className="mb-2 flex flex-wrap items-center justify-between gap-3">
                            <div className="font-mono text-[10px] uppercase tracking-[0.2em] text-slate-300 sm:text-[11px] sm:tracking-[0.28em]">Session {history.length - index} | {formatHistoryDate(item.created_at)}</div>
                          ...
                          <div className="mt-1 break-words font-mono text-xs text-slate-400">{item.overall_score ?? 'No'} / 100 {item.words_per_min ? `| ${item.words_per_min} wpm` : ''}</div>
                        </motion.button>
                      )) : <div className="rounded-[24px] border border-dashed border-white/20 bg-white/4 px-5 py-6 font-mono text-sm text-slate-400">Saved speeches will appear here after the first evaluation.</div>}
                    </div>
                  </Shell>
                  {selectedSession && (
                    <>
