...
import { BookOpen, ChevronRight, History, Menu, Mic, MicOff, RefreshCw, Sparkles, X, Zap } from 'lucide-react';
...
const GLOBAL_CSS = `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  @keyframes pulse-glow { 0%,100% { box-shadow: 0 0 26px rgba(124,58,237,.2); } 50% { box-shadow: 0 0 48px rgba(124,58,237,.38); } }
...
function Background() {
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 0, pointerEvents: 'none' }}>
      ...
      <div style={{ position: 'absolute', inset: 0, backgroundImage: 'linear-gradient(rgba(124,58,237,.05) 1px, transparent 1px), linear-gradient(90deg, rgba(124,58,237,.05) 1px, transparent 1px)', backgroundSize: '58px 58px', opacity: .3 }} />
    </div>
  );
}

function GlassCard({ children, accent = '#7c3aed', style = {} }: { children: React.ReactNode; accent?: string; style?: React.CSSProperties }) {
  return <div style={{ background: 'linear-gradient(135deg, rgba(255,255,255,.045), rgba(255,255,255,.018))', border: `1px solid ${accent}30`, borderLeft: `2px solid ${accent}`, borderRadius: 24, padding: '22px 24px', boxShadow: `0 12px 36px ${accent}10`, backdropFilter: 'blur(22px)', ...style }}>{children}</div>;
}
...
function FeedbackDisplay({ text }: { text: string }) {
  ...
  return (
    <GlassCard accent="#f87171">
      <SectionLabel text="Coach Verdict" color="#f87171" />
      {scoreMatch && <p style={{ color: '#fbbf24', fontSize: 14, fontFamily: "'DM Mono', monospace", marginBottom: 12 }}>Overall score: {scoreMatch[1]}/100</p>}
      <p style={{ color: '#edf2ff', lineHeight: 1.85, whiteSpace: 'pre-wrap', fontSize: 14, fontFamily: "'DM Mono', monospace" }}>{text}</p>
    </GlassCard>
  );
}
...
function GuideCard() {
  const steps = [
    'Choose a template for rubric-based scoring or leave it empty for general ELP and 20-60-20 evaluation.',
    'Record one speech at a time and stop clearly when you finish.',
    'Open older sessions from the history list to revisit transcript and feedback.',
    'Apply the drills, then record again. The coach compares your new attempt against older mistakes.',
  ];
  return (
    <GlassCard accent="#fbbf24">
      <SectionLabel text="How To Use" color="#fbbf24" />
      <div style={{ display: 'grid', gap: 12 }}>
        {steps.map((step, index) => <div key={step} style={{ display: 'flex', gap: 12 }}><div style={{ width: 24, height: 24, borderRadius: '50%', background: 'rgba(251,191,36,.12)', color: '#fde68a', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontFamily: "'DM Mono', monospace" }}>{index + 1}</div><p style={{ color: '#d8def7', lineHeight: 1.75, fontSize: 13, fontFamily: "'DM Mono', monospace" }}>{step}</p></div>)}
      </div>
    </GlassCard>
  );
}
...
function SessionBrowser({ history, selectedSessionId, onSelect }: { history: SpeechHistoryItem[]; selectedSessionId: string | null; onSelect: (id: string) => void }) {
  return (
    <GlassCard accent="#34d399">
      <SectionLabel text="Session History" color="#34d399" />
      <div style={{ display: 'grid', gap: 10 }}>
        {history.length ? history.map((item, index) => {
          const active = item.id === selectedSessionId;
          return <button key={item.id} onClick={() => onSelect(item.id)} style={{ width: '100%', textAlign: 'left', borderRadius: 18, border: `1px solid ${active ? 'rgba(52,211,153,.35)' : 'rgba(148,163,184,.12)'}`, background: active ? 'linear-gradient(135deg, rgba(16,185,129,.14), rgba(124,58,237,.08))' : 'rgba(8,13,18,.52)', padding: '14px 15px', cursor: 'pointer', color: '#e2e8f0' }}><div style={{ display: 'flex', justifyContent: 'space-between', gap: 10, marginBottom: 8 }}><span style={{ color: active ? '#86efac' : '#cbd5e1', fontSize: 11, letterSpacing: 2, fontFamily: "'DM Mono', monospace", textTransform: 'uppercase' }}>Session {history.length - index}</span><span style={{ color: '#94a3b8', fontSize: 11, fontFamily: "'DM Mono', monospace" }}>{formatHistoryDate(item.created_at)}</span></div><p style={{ color: '#d8def7', fontSize: 13, marginBottom: 6 }}>{item.template_label ?? 'General Evaluation'}</p><p style={{ color: '#94a3b8', fontSize: 12, fontFamily: "'DM Mono', monospace" }}>{item.overall_score !== null ? `${item.overall_score}/100` : 'No score'}{item.words_per_min ? ` | ${item.words_per_min} wpm` : ''}</p></button>;
        }) : <div style={{ borderRadius: 18, border: '1px dashed rgba(52,211,153,.2)', padding: '18px 16px', color: '#94a3b8', fontSize: 12, lineHeight: 1.7, fontFamily: "'DM Mono', monospace" }}>Saved speeches will appear here after the first evaluation.</div>}
      </div>
    </GlassCard>
  );
}
...
function SessionPreview({ session }: { session: SpeechHistoryItem | null }) {
  if (!session) return <GlassCard accent="#60a5fa"><SectionLabel text="Session Viewer" color="#60a5fa" /><p style={{ color: '#94a3b8', lineHeight: 1.8, fontSize: 13, fontFamily: "'DM Mono', monospace" }}>Select a saved session to reopen its transcript and feedback.</p></GlassCard>;
  return <div style={{ display: 'grid', gap: 12 }}><GlassCard accent="#60a5fa"><SectionLabel text="Opened Session" color="#60a5fa" /><div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, marginBottom: 12 }}><span style={{ padding: '7px 12px', borderRadius: 999, background: 'rgba(96,165,250,.12)', color: '#bfdbfe', fontSize: 11, fontFamily: "'DM Mono', monospace" }}>{session.template_label ?? 'General Evaluation'}</span><span style={{ padding: '7px 12px', borderRadius: 999, background: 'rgba(124,58,237,.12)', color: '#ddd6fe', fontSize: 11, fontFamily: "'DM Mono', monospace" }}>{formatHistoryDate(session.created_at)}</span></div><p style={{ color: '#d8def7', lineHeight: 1.85, whiteSpace: 'pre-wrap', fontSize: 14, fontFamily: "'DM Mono', monospace" }}>{session.transcript}</p></GlassCard><FeedbackDisplay text={session.feedback} /></div>;
}
...
function MicButton({ isRecording, isAnalyzing, onClick, seconds }: { isRecording: boolean; isAnalyzing: boolean; onClick: () => void; seconds: number }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 18 }}>
      <button onClick={onClick} disabled={isAnalyzing} style={{ width: 138, height: 138, borderRadius: '50%', border: '1px solid rgba(255,255,255,.08)', cursor: isAnalyzing ? 'not-allowed' : 'pointer', color: '#f5f3ff', background: isRecording ? 'linear-gradient(135deg, #4a0d0d, #991b1b)' : 'linear-gradient(135deg, #21104a, #6d28d9, #8b5cf6)', animation: isRecording ? 'record-pulse 2s ease-in-out infinite' : 'pulse-glow 3s ease-in-out infinite', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>{isAnalyzing ? <Sparkles style={{ width: 40, height: 40, animation: 'spin-icon 1.2s linear infinite' }} /> : isRecording ? <MicOff style={{ width: 40, height: 40 }} /> : <Mic style={{ width: 40, height: 40 }} />}</button>
      <div style={{ textAlign: 'center' }}>{isAnalyzing ? <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>{[0, 1, 2].map((i) => <div key={i} style={{ width: 6, height: 6, borderRadius: '50%', background: '#c4b5fd', animation: `dot-bounce 0.8s ${i * 0.2}s ease-in-out infinite` }} />)}<span style={{ fontSize: 11, letterSpacing: 3, color: '#c4b5fd', fontFamily: "'DM Mono', monospace" }}>ANALYSING</span></div> : isRecording ? <><div style={{ fontSize: 36, color: '#fecaca' }}>{label}</div><div style={{ fontSize: 10, color: '#f87171', letterSpacing: 3, fontFamily: "'DM Mono', monospace" }}>TAP TO STOP</div></> : <div style={{ fontSize: 11, color: '#a5b4fc', letterSpacing: 3, fontFamily: "'DM Mono', monospace" }}>TAP TO SPEAK</div>}</div>
    </div>
  );
}

function Hero({ eyebrow, description }: { eyebrow: string; description: string }) {
  return <div style={{ marginBottom: 24 }}><div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14 }}><div style={{ width: 38, height: 1, background: 'linear-gradient(90deg, transparent, #8b5cf6)' }} /><span style={{ fontSize: 10, color: '#c4b5fd', letterSpacing: 4, fontFamily: "'DM Mono', monospace" }}>{eyebrow}</span></div><h1 style={{ fontSize: 'clamp(38px, 5.5vw, 62px)', lineHeight: 1.02, fontWeight: 600, marginBottom: 12 }}>Aawaz Speaker Coach</h1><p style={{ maxWidth: 640, color: '#9ba4c7', fontSize: 15, lineHeight: 1.8, fontFamily: "'DM Mono', monospace" }}>{description}</p></div>;
}

function MetricStrip({ historyCount, selectedTemplateId }: { historyCount: number; selectedTemplateId: SpeechTemplateId | null }) {
  const items = [{ icon: <History style={{ width: 14, height: 14 }} />, label: 'Saved Sessions', value: String(historyCount) }, { icon: <BookOpen style={{ width: 14, height: 14 }} />, label: 'Mode', value: selectedTemplateId ? 'Template' : 'General' }, { icon: <Zap style={{ width: 14, height: 14 }} />, label: 'Feedback', value: 'Brutal' }];
  return <div style={{ display: 'grid', gap: 12, gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))' }}>{items.map((item) => <GlassCard key={item.label} accent="#7c3aed" style={{ padding: '16px 18px' }}><div style={{ display: 'flex', alignItems: 'center', gap: 8, color: '#c4b5fd', marginBottom: 8 }}>{item.icon}<span style={{ fontSize: 11, letterSpacing: 2, fontFamily: "'DM Mono', monospace" }}>{item.label}</span></div><div style={{ color: '#edf2ff', fontSize: 24 }}>{item.value}</div></GlassCard>)}</div>;
}
...
function Sidebar({ activeTab, setActiveTab, open, onClose }: { activeTab: Tab; setActiveTab: (tab: Tab) => void; open: boolean; onClose: () => void }) {
  ...
  return (
    <>
      ...
      <aside style={{ position: 'fixed', top: 0, left: 0, bottom: 0, zIndex: 50, width: 290, padding: 18, background: 'rgba(8,8,18,.94)', borderRight: '1px solid rgba(124,58,237,.14)', backdropFilter: 'blur(24px)', transform: open ? 'translateX(0)' : 'translateX(-100%)', transition: 'transform .3s ease' }} className="md:translate-x-0 md:static md:h-screen">
        ...
        <div style={{ marginTop: 'auto', paddingTop: 18, display: 'grid', gap: 12 }}><GuideCard /><GlassCard accent="#7c3aed"><SectionLabel text="Powered By" color="#a78bfa" /><div style={{ display: 'flex', alignItems: 'center', gap: 8, color: '#ddd6fe', fontSize: 12, fontFamily: "'DM Mono', monospace" }}><Zap style={{ width: 12, height: 12, color: '#fbbf24' }} />GROQ x LLAMA 3.3</div></GlassCard></div>
      </aside>
    </>
  );
}
...
function LivePanel({ isRecording, isAnalyzing, seconds, onMicClick }: { isRecording: boolean; isAnalyzing: boolean; seconds: number; onMicClick: () => void }) {
  return <GlassCard accent="#7c3aed"><SectionLabel text="Live Evaluation" color="#a78bfa" />{isRecording && <div style={{ marginBottom: 20, padding: '16px 0', borderRadius: 18, background: 'rgba(124,58,237,.07)', border: '1px solid rgba(124,58,237,.14)' }}><div style={{ display: 'flex', justifyContent: 'center', gap: 4, height: 48 }}>{Array.from({ length: 30 }).map((_, index) => <div key={index} style={{ width: 4, height: '100%', borderRadius: 99, background: `hsl(${265 + index * 2}, 86%, 74%)`, transformOrigin: 'center', transform: `scaleY(${0.25 + ((index % 7) + 1) / 8})` }} />)}</div></div>}<div style={{ display: 'flex', justifyContent: 'center', padding: '10px 0 6px' }}><MicButton isRecording={isRecording} isAnalyzing={isAnalyzing} onClick={onMicClick} seconds={seconds} /></div></GlassCard>;
}

function Workspace({ eyebrow, title, history, selectedTemplateId, left, right }: { eyebrow: string; title: string; history: SpeechHistoryItem[]; selectedTemplateId: SpeechTemplateId | null; left: React.ReactNode; right: React.ReactNode }) {
  return <div style={{ display: 'grid', gap: 18 }}><Hero eyebrow={eyebrow} description={title} /><MetricStrip historyCount={history.length} selectedTemplateId={selectedTemplateId} /><div style={{ display: 'grid', gap: 18, gridTemplateColumns: 'minmax(0, 1.2fr) minmax(300px, 0.8fr)' }}><div style={{ display: 'grid', gap: 18 }}>{left}</div><div style={{ display: 'grid', gap: 18, alignContent: 'start' }}>{right}</div></div></div>;
}
...
function CoachTab() {
  ...
  return <Workspace eyebrow="SPEAKING COACH" title="Record, review, and reopen older speeches in one place." history={history} selectedTemplateId={selectedTemplateId} left={<><TemplateSelector selectedTemplateId={selectedTemplateId} onChange={setSelectedTemplateId} /><LivePanel isRecording={isRecording} isAnalyzing={isAnalyzing} seconds={seconds} onMicClick={isRecording ? stopRecording : startRecording} />{transcript && <TranscriptDisplay text={transcript} />}{feedback && <FeedbackDisplay text={feedback} />}</>} right={<><SessionBrowser history={history} selectedSessionId={selectedSessionId} onSelect={setSelectedSessionId} /><SessionPreview session={selectedSession} /></>} />;
}
...
function SpeechTab() {
  ...
  const left = <><GlassCard accent="#7c3aed"><SectionLabel text="Generate Topic Speech" color="#a78bfa" /><div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}><input value={topic} onChange={(event) => setTopic(event.target.value)} onKeyDown={(event) => event.key === 'Enter' && generateSpeech()} placeholder="e.g. Leadership, climate change, my role model..." style={{ flex: 1, minWidth: 240, borderRadius: 16, border: '1px solid rgba(124,58,237,.20)', background: 'rgba(124,58,237,.06)', color: '#edf2ff', padding: '15px 18px', fontSize: 14, fontFamily: "'DM Mono', monospace" }} /><button onClick={generateSpeech} disabled={isGenerating || !topic.trim()} style={{ padding: '15px 18px', borderRadius: 16, border: 'none', background: isGenerating || !topic.trim() ? 'rgba(124,58,237,.14)' : 'linear-gradient(135deg, #4c1d95, #7c3aed)', color: isGenerating || !topic.trim() ? '#6b7280' : '#ede9fe', cursor: isGenerating || !topic.trim() ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', gap: 8, fontFamily: "'DM Mono', monospace", fontSize: 12, letterSpacing: 2 }}><Sparkles style={{ width: 15, height: 15, animation: isGenerating ? 'spin-icon 1.2s linear infinite' : 'none' }} />{isGenerating ? 'WRITING...' : 'GENERATE'}</button></div>{error && <p style={{ color: '#f87171', fontSize: 12, fontFamily: "'DM Mono', monospace", marginTop: 12 }}>{error}</p>}</GlassCard><TemplateSelector selectedTemplateId={selectedTemplateId} onChange={setSelectedTemplateId} />{(speech || isGenerating) && <GlassCard accent="#7c3aed"><div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}><SectionLabel text={`Sample Speech${topic ? ` | ${topic}` : ''}`} color="#a78bfa" />{!isGenerating && <button onClick={generateSpeech} style={{ background: 'none', border: 'none', color: '#9ca3af', cursor: 'pointer' }}><RefreshCw style={{ width: 15, height: 15 }} /></button>}</div>{isGenerating ? <div style={{ display: 'flex', gap: 6 }}>{[0, 1, 2].map((i) => <div key={i} style={{ width: 6, height: 6, borderRadius: '50%', background: '#c4b5fd', animation: `dot-bounce 0.8s ${i * 0.2}s ease-in-out infinite` }} />)}</div> : <p style={{ color: '#d8def7', lineHeight: 1.9, whiteSpace: 'pre-wrap', fontSize: 14, fontFamily: "'DM Mono', monospace" }}>{speech}</p>}</GlassCard>}{speech && !isGenerating && <LivePanel isRecording={isRecording} isAnalyzing={isAnalyzing} seconds={seconds} onMicClick={isRecording ? stopRecording : startRecording} />}{transcript && <TranscriptDisplay text={transcript} />}{feedback && <FeedbackDisplay text={feedback} />}</>;
  const right = <><SessionBrowser history={history} selectedSessionId={selectedSessionId} onSelect={setSelectedSessionId} /><SessionPreview session={selectedSession} /><GuideCard /></>;
  return <Workspace eyebrow="SPEECH PRACTICE" title="Generate a sample, rehearse your own version, then compare it with older attempts." history={history} selectedTemplateId={selectedTemplateId} left={left} right={right} />;
}
