...
function CoachTab() {
  ...
  return (
    <div style={{ display: 'grid', gap: 18 }}>
      <Hero eyebrow="SPEAKING COACH" description="Record, get direct technical feedback, and open older speeches from the history list only when you need them. WARNING: AI can make mistakes so dont trust it blindly and if you want to ask something consult me(Aawaz)." />
...
function SpeechTab() {
  ...
  return (
    <div style={{ display: 'grid', gap: 18 }}>
      <Hero eyebrow="SPEECH PRACTICE" description="Generate sample speeches only." />
      <GlassCard accent="#7c3aed">
        <SectionLabel text="Generate Topic Speech" color="#a78bfa" />
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
          <input value={topic} onChange={(event) => setTopic(event.target.value)} onKeyDown={(event) => event.key === 'Enter' && generateSpeech()} placeholder="e.g. Leadership, climate change etc" style={{ flex: 1, minWidth: 240, borderRadius: 16, border: '1px solid rgba(124,58,237,.20)', background: 'rgba(124,58,237,.06)', color: '#edf2ff', padding: '15px 18px', fontSize: 14, fontFamily: "'DM Mono', monospace" }} />
